...
class Robot:
    def __init__(self, id: int, pos: Coordinates, fault: Optional[Literal["crash", "byzantine", "delay"]] = None):
        ...
        self.fault = fault
        self.frozen = False
        print(f"🤖 Robot {id} at ({pos.x:.1f}, {pos.y:.1f}) | Fault: {fault or 'None'}")

    def move(self, target: Coordinates):
        if self.fault == "crash":
            print(f"💥 Robot {self.id} CRASHED and can't move")
            return
...
def main():
    ...
    for step in range(1, 6):  # Now runs 5 steps
        print(f"\n🔄 **STEP {step}**")
        for robot in robots:
            target = Coordinates(
                robot.pos.x + random.uniform(-1, 1),  # Fixed from = to +
